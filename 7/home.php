			...
			<!-- <div class="table-responsive"> -->
				...
				<table class="table table-hover table-responsive">
					...
					<?php
						...
						$tampil = $mysqli->query("SELECT users.id, users.full_name, users.place_of_birth_id, cities.name, users.date_of_birth, users.phone_number,users.address, users.last_education, users.religion, hobbies.name  AS hobby FROM users INNER JOIN cities ON users.place_of_birth_id=cities.id INNER JOIN users_hobbies ON users_hobbies.biodata_id=users.id INNER JOIN hobbies ON users_hobbies.hobby_id=hobbies.id GROUP BY users.id;");
...
?>
			...
			<form method="post" action="aksi_biodata.php?proses=entri">
				...
				<div class="form-group col-sm-6">
					<?php
							include 'koneksi.php';
							$input = $mysqli->query("SELECT * FROM hobbies");
							$no=1;
							while($data_input = $input->fetch_assoc()){
					?>
								<input type="checkbox" name="hobby" value="<?php echo $data_input['id']?>"><?php echo $data_input['name']?><br>
						<?php 
								$no++;
							}
						?>
...
<?php
		...
		case 'edit':
			...
			include 'koneksi.php';
			$tampil = $mysqli->query("SELECT users.id, users.full_name, cities.name, users.place_of_birth_id, users.date_of_birth, users.phone_number, users.address, users.last_education, users.religion, users_hobbies.hobby_id, hobbies.name AS hobby FROM users INNER JOIN cities ON users.place_of_birth_id=cities.id INNER JOIN users_hobbies ON users_hobbies.biodata_id=users.id INNER JOIN hobbies ON users_hobbies.hobby_id=hobbies.id where users.id='$_GET[id_ubah]'");
...
?>
			...
			<form method="post" action="aksi_biodata.php?proses=ubah&id_ubah=<?php echo $data['id']?>">
				...
				<label class="col-sm-2 col-form-label">Hobi</label>
				<div class="form-group col-sm-6">
					<input type="checkbox" name="hobby" value="<?php echo $data['hobby_id']?>" checked><?php echo $data['hobby']?><br>
					<?php
							include 'koneksi.php';
							$input = $mysqli->query("SELECT * FROM hobbies");
							$no=1;
